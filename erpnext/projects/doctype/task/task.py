...
# @frappe.whitelist()
# def get_children(doctype, parent, task=None, project=None,sub_task_category=None,task_category=None, is_root=False):
...
# 	if task_category:
# 		filters.append(["task_category", "=", task_category])
# 	if sub_task_category:
# 		filters.append(["sub_task_category", "=", sub_task_category])  
...
@frappe.whitelist()
def has_tasks(task_category):
    tasks = frappe.get_all("Task", filters={"task_category": task_category}, limit=1)
    return {"has_tasks": bool(tasks)}

import frappe

@frappe.whitelist()
def get_sub_task_categories(task_category):
    if not task_category:
        return []

    query = """
        SELECT task_category_name
        FROM `tabTask Category`
        WHERE parent_task_category = %s
    """
    sub_categories = frappe.db.sql(query, (task_category,), as_dict=True)
    
    # Extract the task_category_name from the results
    return [category['task_category_name'] for category in sub_categories]
...
# @frappe.whitelist()
# def get_children(doctype, parent, task=None, project=None, sub_task_category=None, task_sub_sub_category=None,task_category=None, is_root=False):
#     filters = [["docstatus", "<", "2"]]
...
#         filters.append(["task_sub_sub_category", "=", task_sub_sub_category])        
...
# @frappe.whitelist()
# def get_task_sub_sub_categories(sub_task_category):
#     if not sub_task_category:
#         return []

#     query = """
#         SELECT task_sub_sub_category
#         FROM `tabTask`
#         WHERE sub_task_category = %s
#     """
#     sub_sub_categories = frappe.db.sql(query, (sub_task_category,), as_dict=True)
    
#     # Extract the task_sub_sub_category from the results
#     return [category['task_sub_sub_category'] for category in sub_sub_categories]
